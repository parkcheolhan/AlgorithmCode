package com.example.백준.단계별.기하_직사각형과_삼각형.a9063;
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int x좌측 = 10001, x우측 = -10001, y상위 = -10001, y하위 = 10001;
        int 점의개수 = Integer.parseInt(bufferedReader.readLine());
        for(int i = 0; i < 점의개수 ; i++){
            int[] xy = Arrays.stream(bufferedReader.readLine().split(" ")).mapToInt(Integer::valueOf).toArray();
            x좌측 = Math.min(x좌측, xy[0]);
            x우측 = Math.max(x우측, xy[0]);
            y하위 = Math.min(y하위, xy[1]);
            y상위 = Math.max(y상위, xy[1]);
        }
        System.out.println((x우측-x좌측) * (y상위-y하위));
    }
}
